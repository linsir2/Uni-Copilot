...
def merge_vectors(existing: Optional[List[float]], new: List[float]) -> List[float]:
    ...
    if len(existing) != len(new): return new
    return [(a + b) / 2.0 for a, b in zip(existing, new)]

# --- 2. Robust Qdrant Search ---
def find_point_by_normalized_name(client: QdrantClient, collection: str, norm_name: str) -> Optional[Dict[str, Any]]:
    """
    ...
    try:
        flt = rest.Filter(
            must=[
                rest.FieldCondition(
                    key="normalized_name",
                    match=rest.MatchValue(value=norm_name)
                )
            ]
        )
        ...
        if points:
            ...
            payload = getattr(p, "payload", None) if hasattr(p, "payload") else p.get("payload")
            vector = getattr(p, "vector", None) if hasattr(p, "vector") else p.get("vector")
...
def upsert_entities_to_qdrant(
    ...
    for ent in entities:
        ...
        if existing:
            ...
            final_payload = {**(existing.get("payload") or {}), **props, "name": raw_name, "normalized_name": norm_name}
            
            # Use existing ID to be safe
            point = rest.PointStruct(id=existing["id"], vector=final_vec, payload=final_payload)
            stats["updated"] += 1
        else:
            # INSERT
        ...
        with driver.session() as session:
            for i in range(0, len(entities), batch_size):
                ...
                for e in batch:
                    ...
                    safe_props = {k: v for k, v in e.get("properties", {}).items() 
...
def upsert_entities(
    ...
    """Main Entry Point."""
    logger.info(f"🔄 Upserting {len(entities)} entities (deduplicated by normalized name)...")
    
    q_metrics = upsert_entities_to_qdrant(qdrant_client, qdrant_collection, entities, embed_fn, qdrant_batch_size)
    n_metrics = neo4j_merge_entities(neo4j_uri, neo4j_user, neo4j_password, entities)
    
    logger.info(f"✅ [Qdrant] +{q_metrics['inserted']} / ~{q_metrics['updated']} ({q_metrics['time_ms']}ms)")
    logger.info(f"✅ [Neo4j] Merged: {n_metrics['upserted']} ({n_metrics['time_ms']}ms)")
