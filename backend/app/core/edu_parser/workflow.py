...
class EduMatrixWorkflow(Workflow):
    ...
    async def _rerank_nodes(self, query: str, nodes: List[NodeWithScore], top_n: int, trace_id: str) -> List[NodeWithScore]:
        ...
        with self.lf.start_as_current_observation(
            ...
            data = await self._llm_call(prompt, fallback={"ranked_ids": []}, trace_id=trace_id, step_name="Fallback_Rerank")
            ...
            span.update(output={"method": "LLM_Rerank", "selected_ids": valid_ids})
            return reranked[:top_n]

    # --- Search ---
    async def _tavily_search(self, query: str, trace_id: str) -> List[NodeWithScore]:
        ...
        with self.lf.start_as_current_observation(
            ...
            for attempt in range(2):
                try:
                    ...
                    generation.update(
                        output={"results_count": len(nodes), "answer": data.get("answer")},
                        usage_details={
                            "input": len(query),
                            "total": 1,
                        }
            ...
            generation.update(level="WARNING", status_message="Web Search Failed")
            return []
    ...
    async def retrieve(self, ctx: Context, ev: Union[StartEvent, RewriteEvent]) -> GradeEvent:
        ...
        with self.lf.start_as_current_observation(
            as_type="span",
            trace_context={"trace_id": trace_id},
            input={"search_query": search_query, "is_retry": is_retry},
            name="Retrieve",
        ) as span:
            try:
                ...
                retrieved_docs = [{"text": self._safe_get_content(n)}for n in nodes[:5]]
                span.update(output={"retrieved_nodes": len(nodes), "retrieved_context": retrieved_docs})
    ...
    async def grade(self, ctx: Context, ev: GradeEvent) -> Union[GenerateEvent, RetryRequestEvent, WebSearchEvent]:
        ...
        with self.lf.start_as_current_observation(
            ...
            trace_context={"trace_id": trace_id},
            name="Grade",
            input={"query": ev.query, "nodes_num": len(nodes)}
        ) as span:
            if not nodes:
                self._add_trace_local(trace_id, "Grade", "SKIP", {"reason": "No nodes"}, error_code="NO_CONTENT")
                span.update(level="WARNING", status_message="Not nodes to grade")
            ...
            data = await self._llm_call(prompt, trace_id=trace_id, step_name="Grade", fallback={"score": 0, "reason": "LLM Error", "best_node_idx": -1, "query_type": "DEFAULT"})
            ...
            if last_score is not None and ev.is_retry:
                ...
                span.score(
                    ...
                    value=score_delta,
                    data_type="NUMERIC",
                    comment=f"Strategy: {strat_name} | Reason: {reason}"
                )
            ...
            span.update(output={"score": score, "delta": score_delta, "query_type": query_type})
    ...
    async def _handle_retry(
        ...
        with self.lf.start_as_current_observation(
            as_type="span",
            trace_context={"trace_id": trace_id},
            name="Retry_Routing_Decision",
            input={"score": score, "prev_strategy": prev_strategy, "reason": reason}
        ) as span:
            ...
            if should_retry:
                ...
                span.update(output={"action": "Retry", "next_retry_count": retry_count + 1})
            ...
            span.update(output={"action": "Fallback", "Reason": stop_reason})
    ...
    async def rewrite(self, ctx: Context, ev: RetryRequestEvent) -> RewriteEvent:
        ...
        with self.lf.start_as_current_observation(
            ...
            data = await self._llm_call(prompt, trace_id=trace_id, step_name="Rewrite", fallback={"strategy": "default", "new_query": ev.original_query})
            ...
            span.update(output={"strategy_chosen": strat, "new_query": new_q})
            await self._flush_trace(ctx, trace_id)
    ...
    async def web_search(self, ctx: Context, ev: WebSearchEvent) -> GenerateEvent:
        ...
        nodes = await self._tavily_search(ev.query, trace_id=trace_id)
