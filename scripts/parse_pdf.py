...
2. **Entity Granularity & Cleaning**:
   ...
   - **STRICT PROHIBITION on Pronouns**: Never extract generic pronouns or demonstratives as entities. 
     ...
     - Forbidden Chinese: "它", "这个", "该模型", "此算法", "本节", "该方法".
   - **Coreference Resolution**: If the source text uses a pronoun to refer to a specific concept, you MUST replace it with the full, explicit name of that concept (e.g., replace "it" or "它" with "Convolutional Neural Network").
...
# ==========================================
# 🛠️ Utility: Qdrant Collection Management
# ==========================================
def check_and_create_collection(
    ...
    if recreate and client.collection_exists(collection_name):
        print(f"♻️  Deleting old collection {collection_name} (Recreate Mode)...")
        client.delete_collection(collection_name)

    if not client.collection_exists(collection_name):
        print(f"⚠️ Collection {collection_name} does not exist, creating (Dim: {vector_size})...")
    ...
    def __init__(self, sidecar_path: Path, **kwargs):
        ...
        if sidecar_path.exists():
            try:
                ...
                print(f"📂 Loaded Sidecar Data: {len(self._sidecar_data)} pages found.")
...
def custom_parse_triplets(llm_output: str):
    STOP_CONCEPTS = {
        ...
        "它", "他们", "这个", "那个", "这些", "那些", "该模型", "此算法", 
        "本节", "上图", "下图", "表", "图", "例子", "如下", "这种方法",
    }
    triplets = []
...
async def main(pdf_path_str: str, force_recreate: bool = True):
    ...
    print(f"🚀 [Async] Starting Pipeline: {pdf_path.name}")
    ...
    if force_recreate:
        print("\n🧹 [Clean Start] Recreating collections & Clearing Neo4j...")
        # Clear Qdrant
                ...
                with driver.session() as session:
                    ...
                    for idx_q in index_queries:
                        try:
                            ...
                            print(f"   ⚠️ Index creation skipped (non-critical): {index_err}")
                    
            print("✅ Neo4j cleared and optimized.")
        except Exception as e:
            print(f"Neo4j setup failed: {e}")
    else:
        print("\n⏭️ [Incremental Mode] Skipping collection/DB clearing.")
    ...
    if DYNAMIC_SIDECAR_FILE.exists():
        print("\n🔄 [Upsert] Processing Entities from Sidecar...")
        try:
            ...
            print(f"⚠️ Entity Upsert Failed: {e}")
    ...
    if force_recreate:
        print(f"\n🧠 [Step 1/2] Building Chunk Index ({CHUNK_COLLECTION})...")
        ...
        print(f"\n⏩ [Skip] Chunk Index build skipped (Incremental mode).")

    # 7. Build Graph (Relations & Extensions)
    print(f"\n🕸️ [Step 2/2] Building Knowledge Graph ({ENTITY_COLLECTION})...")
